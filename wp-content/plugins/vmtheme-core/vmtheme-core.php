...
if ( ! defined( 'VM_CORE_VERSION' ) ) {
	define( 'VM_CORE_VERSION', VM_CORE_VERSION );
}
if ( ! defined( 'VM_CORE_DIR' ) ) {
	define( 'VM_CORE_DIR', VM_CORE_DIR );
}
if ( ! defined( 'VM_CORE_URI' ) ) {
	define( 'VM_CORE_URI', VM_CORE_URI );
}
...
if ( ! function_exists( 'vmtheme_core_mime_types' ) ) {
	function vmtheme_core_mime_types( $mimes ) {
...
function vmtheme_core_render_seo_meta_tags() {
	...
	$brand_name = function_exists( 'vmtheme_get_brand_name' ) ? vmtheme_get_brand_name() : ( function_exists( 'vmtheme_get_brand_name' ) ? vmtheme_get_brand_name() : get_bloginfo( 'name' ) );
	$brand_desc = function_exists( 'vmtheme_get_brand_tagline' ) ? vmtheme_get_brand_tagline() : ( function_exists( 'vmtheme_get_brand_tagline' ) ? vmtheme_get_brand_tagline() : get_bloginfo( 'description' ) );
	$current_url = esc_url( ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] );

	$og_title = $brand_name . ' - ' . $brand_desc;
	$og_desc  = $brand_desc;
	$og_type  = 'website';
	$og_image = function_exists( 'vmtheme_get_fallback_backdrop_url' ) ? vmtheme_get_fallback_backdrop_url() : ( function_exists( 'vmtheme_get_fallback_backdrop_url' ) ? vmtheme_get_fallback_backdrop_url() : '' );
	...
	if ( is_singular() ) {
		...
		} elseif ( function_exists( 'vmtheme_get_backdrop_url' ) ) {
			$og_image = vmtheme_get_backdrop_url( $post_id );
		}
	} elseif ( is_tax() ) {
	...
	?>
	<!-- VMTheme Core SEO OpenGraph & Twitter Meta Tags -->
...
if ( ! function_exists( 'vmtheme_core_render_seo_meta_tags' ) ) {
	function vmtheme_core_render_seo_meta_tags() {
...
if ( ! function_exists( 'vmtheme_core_ping_search_engines' ) ) {
	function vmtheme_core_ping_search_engines( $post_id, $post, $update ) {
