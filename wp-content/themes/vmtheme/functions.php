 ...
 * Get Release Date
 */
function vmtheme_get_release_date( $post_id = null, $format = 'M. d, Y' ) {
	if ( ! $post_id ) {
		$post_id = get_the_ID();
	}
	...
	if ( empty( $date ) ) {
		$date = get_post_meta( $post_id, '_vm_first_air_date', true );
	}
	if ( empty( $date ) ) {
		$date = get_post_meta( $post_id, '_doodh_first_air_date', true );
	}

	if ( ! empty( $date ) ) {
		$timestamp = strtotime( $date );
		if ( $timestamp ) {
			return date_i18n( $format ?: get_option( 'date_format', 'F j, Y' ), $timestamp );
		}
	...
	return get_the_date( $format ?: get_option( 'date_format', 'F j, Y' ), $post_id );
}

if ( ! function_exists( 'doodhtheme_get_release_date' ) ) {
	function doodhtheme_get_release_date( $post_id = null, $format = 'M. d, Y' ) {
		return vmtheme_get_release_date( $post_id, $format );
	}
}
...
require_once VMTHEME_DIR . '/inc/backup-importer.php';
