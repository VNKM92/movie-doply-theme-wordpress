...
while ( have_posts() ) :
	...
	<main class="container" style="padding-top: 30px;">
		...
		<!-- Video Player (Conditional on Enable/Disable Setting) -->
		<?php if ( doodhtheme_is_player_enabled( $ep_id ) ) : ?>
			<section class="doodh-section" id="doodh-episode-player-section">
		...
		<?php else : ?>
			<!-- Navigation Bar when player is disabled -->
			<div style="display:flex; justify-content:space-between; align-items:center; background:var(--dt-bg-surface); padding:12px 18px; border:1px solid var(--dt-border); border-radius:var(--dt-radius); margin-bottom:25px; flex-wrap:wrap; gap:10px;">
				<div>
					<?php if ( ! empty( $prev_ep ) ) : ?>
						<a href="<?php echo esc_url( get_permalink( $prev_ep[0]->ID ) ); ?>" class="doodh-btn-secondary" style="padding:8px 16px; font-size:13px;">
							<i class="fas fa-step-backward"></i> <?php esc_html_e( 'Previous Episode', 'vmtheme' ); ?>
