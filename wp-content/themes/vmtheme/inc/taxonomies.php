...
/**
 * Add Custom Fields to Genres Taxonomy (Add Screen)
 */
function doodhtheme_genres_add_form_fields() {
	?>
	<div class="form-field">
		<label for="genre_bottom_title"><?php esc_html_e( 'Bottom Section Title', 'vmtheme' ); ?></label>
		<input type="text" name="genre_bottom_title" id="genre_bottom_title" value="" placeholder="<?php esc_attr_e( 'e.g. About Action Movies & TV Shows', 'vmtheme' ); ?>">
		<p class="description"><?php esc_html_e( 'Custom heading displayed at the bottom content section of this genre page.', 'vmtheme' ); ?></p>
	</div>
	<div class="form-field">
		<label for="genre_bottom_desc_add"><?php esc_html_e( 'Bottom Section Description / Content', 'vmtheme' ); ?></label>
		<?php
		$settings = array(
			'textarea_name' => 'genre_bottom_description',
			'textarea_rows' => 8,
			'media_buttons' => true,
			'tinymce'       => true,
			'quicktags'     => true,
		);
		wp_editor( '', 'genrebottomdescadd', $settings );
		?>
		<p class="description"><?php esc_html_e( 'Rich content (SEO text, guide, FAQs) displayed at the bottom of this genre archive page.', 'vmtheme' ); ?></p>
	</div>
	<?php
}
add_action( 'genres_add_form_fields', 'doodhtheme_genres_add_form_fields' );

/**
 * Edit Custom Fields on Genres Taxonomy (Edit Screen)
 */
function doodhtheme_genres_edit_form_fields( $term ) {
	...
	?>
	<tr class="form-field">
		<th scope="row"><label for="genre_bottom_title"><?php esc_html_e( 'Bottom Section Title', 'vmtheme' ); ?></label></th>
		<td>
			<input type="text" name="genre_bottom_title" id="genre_bottom_title" value="<?php echo esc_attr( $bottom_title ); ?>" class="large-text" placeholder="<?php esc_attr_e( 'e.g. Best Action Movies & Series Overview', 'vmtheme' ); ?>">
			<p class="description"><?php esc_html_e( 'Custom title displayed at the bottom content section of this genre page.', 'vmtheme' ); ?></p>
		</td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="genre_bottom_desc_edit"><?php esc_html_e( 'Bottom Section Description (Rich Text)', 'vmtheme' ); ?></label></th>
		<td>
			<?php
			$settings = array(
				'textarea_name' => 'genre_bottom_description',
			...
			wp_editor( wp_specialchars_decode( $bottom_desc, ENT_QUOTES ), 'genrebottomdescedit', $settings );
			?>
			<p class="description"><?php esc_html_e( 'Rich text content (SEO overview, recommended watch guides, FAQs) displayed dynamically at the bottom of this genre page in frontend. You can use full formatting, links, bullet points, and media.', 'vmtheme' ); ?></p>
		</td>
	</tr>
	<?php
}
add_action( 'genres_edit_form_fields', 'doodhtheme_genres_edit_form_fields' );

/**
 * Save Genre Taxonomy Custom Fields
 */
function doodhtheme_save_genres_meta( $term_id ) {
	if ( isset( $_POST['genre_bottom_title'] ) ) {
		update_term_meta( $term_id, '_dt_genre_bottom_title', sanitize_text_field( wp_unslash( $_POST['genre_bottom_title'] ) ) );
	}
	if ( isset( $_POST['genre_bottom_description'] ) ) {
		update_term_meta( $term_id, '_dt_genre_bottom_description', wp_kses_post( wp_unslash( $_POST['genre_bottom_description'] ) ) );
	}
}
add_action( 'created_genres', 'doodhtheme_save_genres_meta' );
add_action( 'edited_genres', 'doodhtheme_save_genres_meta' );
