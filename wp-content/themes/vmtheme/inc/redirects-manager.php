...
function doodhtheme_register_redirects_menu() {
	add_theme_page(
		__( '301 Redirects & 404 Manager', 'vmtheme' ),
		__( '301 & 404 Redirects', 'vmtheme' ),
		'manage_options',
...
function doodhtheme_render_redirects_manager_page() {
	...
	if ( isset( $_POST['doodh_add_redirect'] ) && check_admin_referer( 'doodh_redirect_nonce' ) ) {
		...
		if ( $source && $target ) {
			...
			echo '<div class="notice notice-success is-dismissible"><p><strong>' . esc_html__( 'Redirect rule added successfully!', 'vmtheme' ) . '</strong></p></div>';
		}
	}
	...
	if ( isset( $_GET['action'] ) && $_GET['action'] === 'delete' && isset( $_GET['rule_id'] ) && check_admin_referer( 'doodh_del_rule_' . $_GET['rule_id'] ) ) {
		...
		echo '<div class="notice notice-success is-dismissible"><p><strong>' . esc_html__( 'Redirect rule deleted.', 'vmtheme' ) . '</strong></p></div>';
	}
	...
	if ( isset( $_POST['doodh_save_404_settings'] ) && check_admin_referer( 'doodh_404_settings_nonce' ) ) {
		...
		echo '<div class="notice notice-success is-dismissible"><p><strong>' . esc_html__( '404 auto-healing settings updated!', 'vmtheme' ) . '</strong></p></div>';
	}
	...
	if ( isset( $_POST['doodh_clear_404_logs'] ) && check_admin_referer( 'doodh_clear_logs_nonce' ) ) {
		...
		echo '<div class="notice notice-success is-dismissible"><p><strong>' . esc_html__( '404 error logs cleared.', 'vmtheme' ) . '</strong></p></div>';
	}
	...
	<div class="wrap" style="max-width:1100px; margin-top:20px;">
		<div style="background:#111827; color:#fff; padding:24px; border-radius:10px; margin-bottom:25px; box-shadow:0 4px 15px rgba(0,0,0,0.15);">
			<h1 style="color:#fff; margin:0 0 8px; font-size:24px; display:flex; align-items:center; gap:10px;">
				...
				<?php esc_html_e( 'Broken URL 301 Redirects & 404 Healing Manager', 'vmtheme' ); ?>
			</h1>
			<p style="color:#94a3b8; margin:0; font-size:14px;">
				<?php esc_html_e( 'Create 301/302 redirects for broken or deleted URLs, automatically heal 404 errors, and track visitor 404 logs.', 'vmtheme' ); ?>
			</p>
		</div>
		...
		<div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
			...
			<div style="background:#fff; padding:20px; border-radius:8px; border:1px solid #e2e8f0;">
				<h3 style="margin-top:0; border-bottom:1px solid #e2e8f0; padding-bottom:10px;">
					<i class="dashicons dashicons-plus-alt"></i> <?php esc_html_e( 'Add New Redirect Rule', 'vmtheme' ); ?>
				</h3>
				<form method="post" action="">
					<?php wp_nonce_field( 'doodh_redirect_nonce' ); ?>
					<p>
						<label><strong><?php esc_html_e( 'Source Relative Path (Broken URL):', 'vmtheme' ); ?></strong></label><br>
						<input type="text" name="redirect_source" placeholder="/old-movie-slug/ or /movies/dead-link" class="large-text" required>
					</p>
					<p>
						<label><strong><?php esc_html_e( 'Target Destination URL:', 'vmtheme' ); ?></strong></label><br>
						<input type="text" name="redirect_target" placeholder="<?php echo esc_url( home_url( '/movies/' ) ); ?>" class="large-text" required>
					</p>
					<p>
						<label><strong><?php esc_html_e( 'Redirect Type:', 'vmtheme' ); ?></strong></label><br>
						<select name="redirect_code" style="width:100%;">
							<option value="301" selected><?php esc_html_e( '301 Permanent (Recommended for SEO)', 'vmtheme' ); ?></option>
							<option value="302"><?php esc_html_e( '302 Temporary', 'vmtheme' ); ?></option>
						</select>
					</p>
					<p>
						<input type="submit" name="doodh_add_redirect" class="button button-primary" value="<?php esc_attr_e( 'Save Redirect Rule', 'vmtheme' ); ?>">
					</p>
				</form>
			</div>
			...
			<div style="background:#fff; padding:20px; border-radius:8px; border:1px solid #e2e8f0;">
				<h3 style="margin-top:0; border-bottom:1px solid #e2e8f0; padding-bottom:10px;">
					<i class="dashicons dashicons-shield-alt"></i> <?php esc_html_e( '404 Auto-Healing Options', 'vmtheme' ); ?>
				</h3>
				<form method="post" action="">
					<?php wp_nonce_field( 'doodh_404_settings_nonce' ); ?>
					<p>
						<label><strong><?php esc_html_e( 'Auto-Redirect 404 Broken URLs:', 'vmtheme' ); ?></strong></label><br>
						<select name="doodh_auto_404_redirect" style="width:100%;">
							<option value="no" <?php selected( $auto_404, 'no' ); ?>><?php esc_html_e( 'Disabled (Display standard 404 page)', 'vmtheme' ); ?></option>
							<option value="yes" <?php selected( $auto_404, 'yes' ); ?>><?php esc_html_e( 'Enabled (301 Redirect all 404s to target URL)', 'vmtheme' ); ?></option>
						</select>
					</p>
					<p>
						<label><strong><?php esc_html_e( 'Fallback Target URL for 404s:', 'vmtheme' ); ?></strong></label><br>
						<input type="text" name="doodh_404_target_url" value="<?php echo esc_attr( $target_404 ); ?>" class="large-text">
					</p>
					<p>
						<input type="submit" name="doodh_save_404_settings" class="button button-secondary" value="<?php esc_attr_e( 'Update 404 Settings', 'vmtheme' ); ?>">
					</p>
				</form>
			</div>
		</div>
		...
		<div style="background:#fff; padding:20px; border-radius:8px; border:1px solid #e2e8f0; margin-top:25px;">
			<h3 style="margin-top:0;">
				<i class="dashicons dashicons-list-view"></i> <?php esc_html_e( 'Active 301 / 302 Redirect Rules', 'vmtheme' ); ?> (<?php echo count( $rules ); ?>)
			</h3>
			<table class="widefat striped">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Source Path', 'vmtheme' ); ?></th>
						<th><?php esc_html_e( 'Target Destination', 'vmtheme' ); ?></th>
						<th><?php esc_html_e( 'Code', 'vmtheme' ); ?></th>
						<th><?php esc_html_e( 'Date Added', 'vmtheme' ); ?></th>
					...
					<?php else : ?>
						<tr>
							<td colspan="5" style="text-align:center; padding:15px; color:#64748b;">
								<?php esc_html_e( 'No custom redirect rules created yet.', 'vmtheme' ); ?>
							</td>
						</tr>
					<?php endif; ?>
				</tbody>
		...
		<div style="background:#fff; padding:20px; border-radius:8px; border:1px solid #e2e8f0; margin-top:25px;">
			<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
				<h3 style="margin:0;"><i class="dashicons dashicons-warning"></i> <?php esc_html_e( 'Live 404 Error Log (Broken Links Encountered by Users & Bots)', 'vmtheme' ); ?></h3>
				<form method="post" action="" style="margin:0;">
					<?php wp_nonce_field( 'doodh_clear_logs_nonce' ); ?>
					<input type="submit" name="doodh_clear_404_logs" class="button button-small" value="<?php esc_attr_e( 'Clear 404 Logs', 'vmtheme' ); ?>" onclick="return confirm('Clear all recorded 404 logs?');">
				</form>
			</div>
			...
			<table class="widefat striped">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Broken URL Requested', 'vmtheme' ); ?></th>
						<th><?php esc_html_e( 'Hits Count', 'vmtheme' ); ?></th>
						<th><?php esc_html_e( 'Referrer', 'vmtheme' ); ?></th>
						<th><?php esc_html_e( 'Last Encountered', 'vmtheme' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php if ( ! empty( $logs ) ) : ?>
					...
					<?php else : ?>
						<tr>
							<td colspan="4" style="text-align:center; padding:15px; color:#22c55e;">
								<i class="dashicons dashicons-yes-alt"></i> <?php esc_html_e( 'Clean Record! No 404 errors logged.', 'vmtheme' ); ?>
							</td>
						</tr>
					<?php endif; ?>
				</tbody>
